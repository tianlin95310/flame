...
class MulFileSkill extends PositionComponent with HasPaint, BasicSkill {
  ...
  @override
  void render(Canvas canvas) {
    frames[flyingSpriteIndex.toInt()]
        .renderRect(canvas, Rect.fromLTRB(0, 0, size.x, size.y));
  }
...
class SkillLoadUtils {
  ...
  static Future<PositionComponent> loadCommon(name, frameCount) async {
    ...
    for (int i = 0; i < frameCount; i++) {
      ...
      skill.frames.add(Sprite(await Flame.images
          .load('models/mulSkill/' + name + '/s${fileName}.png')));
    }
    return skill;
  }
  ...
  static PositionComponent skillSingleFile(file, row, col, startIndex, endIndex,
